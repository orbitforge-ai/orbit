...
export function PermissionPrompt({
  ...
  if (localResolved) {
    ...
    return (
      <div
        className={`rounded-lg border ${isAllowed ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-red-500/30 bg-red-500/5'} px-3 py-2 flex items-center gap-2`}
      >
        {isAllowed ? (
  ...
  return (
    <div className={`rounded-lg border ${borderColor} ${bgColor} overflow-hidden`}>
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <span
          className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${isDangerous ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400'}`}
        >
