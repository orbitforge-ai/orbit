...
const CONCURRENCY_OPTIONS: { value: string; label: string; hint: string }[] = [
  ...
  { value: "cancel_previous", label: "Cancel previous", hint: "Stop the active run and start the new one" },
];
  ...
  function buildConfig() {
    ...
    if (kind === "http_request") {
      ...
      if (httpExpectedCodes.trim()) {
        cfg.expectedStatusCodes = httpExpectedCodes.split(",").map(s => parseInt(s.trim(), 10)).filter(Boolean);
      }
      return cfg;
    }
    if (kind === "agent_step") {
            ...
            {kind === "script_file" && (
              <>
                <Field label="Script path">
                  <input type="text" value={scriptPath} onChange={e => setScriptPath(e.target.value)}
                    placeholder="/Users/you/scripts/backup.sh" className={inputCls} />
                </Field>
                <Field label="Interpreter">
                  <input type="text" value={interpreter} onChange={e => setInterpreter(e.target.value)}
                    placeholder="/bin/sh" className={inputCls} />
                </Field>
                <Field label="Working directory (optional)">
                  <input type="text" value={workingDir} onChange={e => setWorkingDir(e.target.value)}
                    placeholder="~/scripts" className={inputCls} />
                  ...
                  <div className="space-y-2">
                    {httpHeaders.map((h, i) => (
                      <div key={i} className="flex gap-2 items-center">
                        <input type="text" placeholder="Header-Name" value={h.k}
                          onChange={e => setHttpHeaders(prev => prev.map((x, j) => j === i ? { ...x, k: e.target.value } : x))}
                          className={`${inputCls} flex-1`} />
                        <input type="text" placeholder="value" value={h.v}
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Max retries">
                <input type="number" min={0} max={10} value={maxRetries}
                  onChange={e => setMaxRetries(Number(e.target.value))} className={inputCls} />
              </Field>
              <Field label="Retry delay (sec)">
                <input type="number" min={10} value={retryDelaySecs}
                  onChange={e => setRetryDelaySecs(Number(e.target.value))} className={inputCls} />
              </Field>
            </div>
          </>
        )}
          ...
          <>
            ...
            {scheduleKind === "one_shot" && (
              <div className="space-y-4">
                <Field label="Date">
                  <input type="date" value={oneShotDate} onChange={e => setOneShotDate(e.target.value)}
                    className={inputCls} />
                </Field>
                <Field label="Time">
                  <input type="time" value={oneShotTime} onChange={e => setOneShotTime(e.target.value)}
                    className={inputCls} />
                </Field>
                {oneShotDate && (
