...
mod http {
    ...
    pub fn register(reg: &mut crate::shim::registry::Registry) {
        ...
        reg.register("complete_work_item", |ctx, args| async move {
            ...
            let a: IdArgs = serde_json::from_value(args).map_err(|e| e.to_string())?;
            let r =
                complete_work_item(a.id, app.state::<DbPool>(), app.state::<CloudClientState>())
                    .await?;
            serde_json::to_value(r).map_err(|e| e.to_string())
        });
        ...
        reg.register("create_work_item_comment", |ctx, args| async move {
            ...
            let r = create_work_item_comment(
                a.work_item_id,
                a.body,
                a.author,
                app.state::<DbPool>(),
        ...
        reg.register("update_work_item_comment", |ctx, args| async move {
            ...
            let r = update_work_item_comment(
                a.id,
                a.body,
                app.state::<DbPool>(),
                app.state::<CloudClientState>(),
        ...
        reg.register("delete_work_item_comment", |ctx, args| async move {
            let app = ctx.app()?;
            let a: IdArgs = serde_json::from_value(args).map_err(|e| e.to_string())?;
            delete_work_item_comment(a.id, app.state::<DbPool>(), app.state::<CloudClientState>())
                .await?;
            Ok(serde_json::Value::Null)
        });
    }
