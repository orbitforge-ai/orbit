...
pub async fn execute_tool(
    ...
    match tool_name {
        "shell_command" => {
            ...
            let result =
                execute_shell_command(&ctx.workspace_root, command, timeout_secs, app, run_id)
                    .await?;
        ...
        if !exists {
          return Err(format!("Message '{}' not found or not a user message in this session", msg_id));
        }
