...
use crate::db::DbPool;
use crate::events::emitter::{emit_agent_iteration, emit_log_chunk};
...
pub async fn run_agent_loop(
    ...
    let model = cfg
        .model
        .clone()
        .unwrap_or_else(|| ws_config.model.clone());
    ...
    // ── Load system prompt ───────────────────────────────────────────────
    let system_prompt = workspace::read_workspace_file(agent_id, "system_prompt.md")
        .unwrap_or_else(|_| "You are a helpful autonomous agent.".to_string());

    // ── Resolve provider + API key ───────────────────────────────────────
    let provider_name = &ws_config.provider;
    let api_key = keychain::retrieve_api_key(provider_name).map_err(|e| {
        let msg = format!(
    ...
    let tools: Vec<ToolDefinition> =
        agent_tools::build_tool_definitions(&ws_config.allowed_tools);
            ...
            llm_provider::StopReason::ToolUse => {
                ...
                for block in &response.content {
                    if let ContentBlock::ToolUse { id, name, input } = block {
                        ...
                        match agent_tools::execute_tool(
                            &tool_ctx, name, input, app, run_id,
                        )
                        .await
                        {
                            Ok((result, is_finish)) => {
...
fn save_conversation_to_db(
    ...
    if let Ok(conn) = db.get() {
        ...
        let _ = conn.execute(
            ...
            rusqlite::params![id, agent_id, run_id, messages_json, input_tokens, output_tokens, iterations, now],
        );
    }
}
