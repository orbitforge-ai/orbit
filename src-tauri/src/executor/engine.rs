...
impl ActiveRunRegistry {
  ...
  pub async fn register(&self, agent_id: &str, run_id: &str, cancel_tx: oneshot::Sender<()>) {
    ...
    active.entry(agent_id.to_string()).or_default().insert(run_id.to_string());
    drop(active);
...
async fn run_one(
  ...
  let result = match task.kind.as_str() {
    "shell_command" => {
      let cfg: ShellCommandConfig = serde_json
        ::from_value(task.config.clone())
        .map_err(|e| e.to_string())?;
      process::run_shell(&run_id, &cfg, &log_path, timeout_secs, &app, cancel).await
    }
    "script_file" => {
      let cfg: ScriptFileConfig = serde_json
        ::from_value(task.config.clone())
        .map_err(|e| e.to_string())?;
      process::run_script(&run_id, &cfg, &log_path, timeout_secs, &app, cancel).await
    }
    "http_request" => {
      let cfg: HttpRequestConfig = serde_json
        ::from_value(task.config.clone())
        .map_err(|e| e.to_string())?;
      http
        ::run_http(&run_id, &cfg, &log_path, timeout_secs, &app, cancel).await
    ...
    "agent_step" => {
      let cfg: AgentStepConfig = serde_json
        ::from_value(task.config.clone())
        .map_err(|e| e.to_string())?;
      let agent_id = task.agent_id.clone().unwrap_or_else(|| DEFAULT_AGENT_ID.to_string());
    ...
    "agent_loop" => {
      let cfg: AgentLoopConfig = serde_json
        ::from_value(task.config.clone())
        .map_err(|e| e.to_string())?;
      let agent_id = task.agent_id.clone().unwrap_or_else(|| DEFAULT_AGENT_ID.to_string());
...
async fn schedule_retry_if_needed(
  ...
  let state: Option<String> = conn
    .query_row("SELECT state FROM runs WHERE id = ?1", rusqlite::params![req.run_id], |row|
      row.get(0)
    )
    .ok();
...
async fn evaluate_bus_subscriptions(
  ...
  let final_state = {
    ...
    match tokio::task::spawn_blocking(move || {
      let conn = pool.get().ok()?;
      conn.query_row("SELECT state FROM runs WHERE id = ?1", rusqlite::params![rid], |row| row.get::<_, String>(0)).ok()
    }).await {
      Ok(Some(s)) => s,
      _ => return,
    }
  ...
  if !matches!(final_state.as_str(), "success" | "failure" | "timed_out" | "cancelled") {
    return;
  }
  ...
  let subscriptions = match tokio::task::spawn_blocking(move || -> Result<Vec<(String, String, String, String, i64)>, String> {
