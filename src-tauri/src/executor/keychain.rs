...
/// Generic secret store. Used by the plugin system (service names scoped per
/// plugin) and by the API-key helpers below.
...
pub fn retrieve_secret(service: &str, account: &str) -> Result<String, String> {
    ...
    if output.status.success() {
        ...
        Err(format!(
            "no secret found for service {:?} account {:?}",
            service, account
        ))
    }
}
...
pub fn store_api_key(provider: &str, key: &str) -> Result<(), String> {
    let result = store_secret(SERVICE_NAME, provider, key);
    if result.is_ok() {
...
pub fn retrieve_api_key(provider: &str) -> Result<String, String> {
    ...
    retrieve_secret(SERVICE_NAME, provider)
        .map_err(|_| format!("no API key found for provider: {}", provider))
}

/// Delete an API key from the macOS Keychain.
pub fn delete_api_key(provider: &str) -> Result<(), String> {
    match delete_secret(SERVICE_NAME, provider) {
        Ok(()) => {
            info!(provider = provider, "API key deleted from Keychain");
            Ok(())
        }
        Err(e) => {
            warn!(provider = provider, err = %e, "failed to delete API key");
            Err(e)
        }
    }
}
