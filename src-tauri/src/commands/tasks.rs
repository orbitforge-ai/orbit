...
pub async fn create_task(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let tags_str = serde_json::to_string(&payload.tags.unwrap_or_default())
            .map_err(|e| e.to_string())?;
        ...
        let agent_id = payload
            .agent_id
            .unwrap_or_else(|| "01HZDEFAULTDEFAULTDEFAULTDA".to_string());
        ...
        conn.execute(
            ...
            rusqlite::params![
                id, payload.name, payload.description, payload.kind, config_str,
                max_duration, max_retries, retry_delay, concurrency, tags_str,
                agent_id, now,
            ],
        )
        .map_err(|e| e.to_string())?;
